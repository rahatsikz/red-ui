...
const ToastComponent = ({ message, type, onDismiss, placement }: any) => {
  ...
  return visible ? (
    <div
      className={`fixed ${placement.class} z-30 text-white px-4 py-2 rounded-md ${toastStyles[type]}`}
    >
  ...
  const toastPlacementOptions = [
    {
      label: "Top",
      value: "top",
      class: "top-16 left-1/2 -translate-x-1/2 ",
    },
    {
      label: "Bottom",
      value: "bottom",
      class: "bottom-16 left-1/2 -translate-x-1/2",
    },
    {
      label: "Right",
      value: "right",
      class: "right-4 lg:right-16 bottom-16",
    },
  ];

  const [toastPlacement, setToastPlacement] = useState<any>(
    toastPlacementOptions[0]
  );

  const handleToastPlacementChange = (value: any) => {
    setToastPlacement(value);
  };
...
const ToastComponent = ({ message, type, onDismiss }: ToastProps) => {
  const [visible, setVisible] = useState(false);
  ...
  return visible ? (
    <div className={"fixed ${toastPlacement.class} z-30 text-white px-4 py-2 rounded-md " + toastStyles[type]}>
      <div className='flex items-center gap-2'>
        {type === "success" ? (
          <div className='w-6'>
            <svg
...
const HowToUseToast = () => {
  ...
  useEffect(() => {
    setCode("");
  }, [toastType, message, toastPlacement]);
